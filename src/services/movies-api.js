...
const KEY = '904ef6f007cb228dd6ac008b3d697336';
...
export const getMovieCast = async movieId => {
  const queryString = `movie/${movieId}/credits?api_key=${KEY}&language=en-US`; // Poprawna składnia
  const { data } = await axios.get(queryString);
  return data;
};

export const getReviews = async movieId => {
  const queryString = `movie/${movieId}/reviews?api_key=${KEY}&language=en-US`; // Poprawna składnia
  const { data } = await axios.get(queryString);
  return data;
};
